...
print("Gestartet. Ãœberwache auf: " .. table.concat(targetPlayers, ", "))
...
while true do
    ...
    -- 2. Logout prÃ¼fen (Vergleich: War gespeichert, ist jetzt NICHT mehr online)
    local logoutMsg = ""
    for name, data in pairs(lastKnownData) do
        ...
        if not currentOnline[name] then
            -- PrÃ¼fen ob es ein Target-Spieler war
            local isTarget = false
            for _, t in ipairs(targetPlayers) do if name == t then isTarget = true end end
            
            if isTarget then
                -- HIER nutzen wir die GESPEICHERTE Position aus 'data', nicht vom Detector!
                logoutMsg = logoutMsg .. "âš ï¸ **" .. name .. " hat sich ausgeloggt.**\n"
                logoutMsg = logoutMsg .. "   Letzte Position: X: " .. data.x .. " Y: " .. data.y .. " Z: " .. data.z .. " (" .. data.dim .. ")\n\n"
                
                -- Erst NACH der Nachricht lÃ¶schen, damit die Daten fÃ¼r die Meldung erhalten bleiben
    ...
    -- Nur senden wenn sich was geÃ¤ndert hat oder alle 10 Sekunden (wie vorher)
