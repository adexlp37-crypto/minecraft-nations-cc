-- KONFIGURATION
local apiKey = "REPLACE_WITH_GROQ_API_KEY" -- HIER DEINEN KEY EINFÃœGEN
...
local function askGroq(userPrompt)
    -- FÃ¼ge den aktuellen Prompt hinzu (wurde schon in der Schleife gemacht, aber sicherheitshalber hier)
...
while true do
    ...
    -- Formatieren der Nachricht fÃ¼r die History
    local chatEntry = "Player " .. spieler .. " says: " .. nachricht
    
    -- 1. IMMER SPEICHERN (Passives MithÃ¶ren)
    -- Jede Nachricht im Chat wird zur History hinzugefÃ¼gt, egal ob Jarvis gerufen wird
    table.insert(conversationHistory, { role = "user", content = chatEntry })
    
    -- Speicher begrenzen (Wichtig! Nur letzte 10 EintrÃ¤ge behalten)
    if #conversationHistory > 12 then
        table.remove(conversationHistory, 2) -- LÃ¶sche Ã¤ltesten User-Eintrag
        -- Falls der nÃ¤chste Eintrag eine AI-Antwort war, diese auch lÃ¶schen, um Konsistenz zu wahren
    ...
    if string.find(string.lower(nachricht), triggerWord) then
        ...
        -- Ãœberschreibe den letzten Eintrag in der History mit dem bereinigten Prompt,
        -- damit die KI nicht denkt, der Spieler hÃ¤tte "Jarvis" im Satzteil gesagt, den sie analysieren soll.
        conversationHistory[#conversationHistory].content = finalPrompt

        print("Jarvis wird gerufen von " .. spieler .. "...")
        
        local antwort = askGroq() -- Keine Ãœbergabe mehr nÃ¶tig, History ist global aktuell
